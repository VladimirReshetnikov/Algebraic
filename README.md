...
| `root-decomposition/WOLFRAM-NOTES.md` | Subtle Wolfram Language behaviour discovered while developing and running the code. |
| `LICENSE` | MIT-0. |
...
are computed, where the components may be any algebraic numbers.  Sums have
...
rigorous lower bounds; no complete algorithm is known.  Both examples of the
...
Python (from `root-decomposition/python/`, needs `python-flint`):
