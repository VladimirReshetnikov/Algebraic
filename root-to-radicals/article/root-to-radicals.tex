...
$5$-cycle is found within the first few primes and nonsolvability is proved in about
$10$ milliseconds.
...
$1000$ digits and recognizes with \code{RootApproximant} as an algebraic number of
moderate degree; the notebook then tries to express that number through nested
radicals and $\cos(\pi/9)$.  Its devices, in the order they appear, are:
...
$\Q\subset\Q(3^{1/6})\subset\Q(3^{1/6},\theta)$ over which the target becomes quadratic;
what it lacks is a termination guarantee and a criterion for when to stop looking.  The
descent of Section~\ref{sec:descent} supplies both, at the price of the splitting field.
