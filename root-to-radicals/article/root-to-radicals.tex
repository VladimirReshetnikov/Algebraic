...
Composite prime-power degrees need one more step.

\begin{proposition}\label{prop:primepower}
Let $n=p^k$ with $k\ge2$.  If $G$ contains a cycle of prime length $\ell$ with
$n/2<\ell<n-1$ (all other points fixed), then $G$ is not solvable.
\end{proposition}

\begin{proof}
As in Proposition~\ref{prop:longcycle}, $G$ is primitive, and if it were solvable it
would have a regular normal elementary abelian subgroup $V\cong\F_p^k$ with
$G\le\AGL(k,p)=V\rtimes\operatorname{GL}(k,p)$ acting on $V$.  The $\ell$-cycle $g$ fixes
a point; conjugating by a translation we may take that point to be $0$, so
$g\in\operatorname{GL}(k,p)$, and the fixed points of $g$ form a subspace, of size $p^j$
for some $j$.  The moved points are the single $\ell$-cycle, so $p^k-p^j=\ell$.  Since
$\ell$ is prime and $p^j$ divides the left side, $j=0$ and $\ell=n-1$, contrary to
$\ell<n-1$.
\end{proof}

The excluded length $\ell=n-1$ does occur in solvable groups: $\AGL(1,8)$, of order $56$
in degree $8$, contains $7$-cycles, and $S_4$ contains $3$-cycles.  For $x^8-x-1$ the
implementations find a $5$-cycle and for $x^9-x-1$ a $5$- or $7$-cycle within the first
few dozen primes, and prove nonsolvability in well under a second.

All three criteria are inconclusive in the other direction (they never prove
solvability), and a nonsolvable group may fail to reveal itself through these particular
cycle types; in those cases the implementations compute the group exactly, subject to the
group order limit.  A cheap companion test avoids the expensive failure mode: the least
common multiple of the orders of the Frobenius elements found divides $|G|$, and when it
already exceeds the group order limit the status \code{ResourceLimit} is returned before
any splitting field is built.  For
...
over $\Q(3^{1/6},\theta)$ into three cubics ($4$ minutes), so $\beta_0$ is a root of a
cubic with radical coefficients.  The package assembles the expression with the
\code{"Extension"} option, which does exactly this (factor cumulatively over the given
generators, solve the factor containing the input by the cubic formula with $\theta$ kept
as an atom, select the branch numerically, and substitute the radical expression of
$\theta$ once at the end): \code{NotebookTarget.wl} produces in $11$ minutes a radical
expression for $\beta_0$ of depth $5$ with $107\,067$ leaves, equal to the inverse beta
value to $300$ digits.  Its exactness rests on the construction (an exact factor, the
exact cubic formula, and a branch selected among three separated candidates), because
\code{RootReduce} on an expression of this size did not finish within the $5$-minute
verification cap; the report therefore carries \code{Verified -> Indeterminate}.  Two
things went wrong on the way and are worth recording: factoring the degree-$27$
polynomial over the degree-$54$ field in one step did not finish in an hour, whereas
factoring in two stages takes $4$ minutes; and recognizing which cubic factor vanishes at
$\beta_0$ requires evaluating coefficients of height $10^{100}$ at a precision that
exceeds their cancellation, since a $40$-digit evaluation leaves a residual of size
$10^{60}$ for every factor.  The notebook's devices, in the order they appear, are:
...
produces the radical expression directly; the \code{"Extension"} option then completes
...
\code{"VerificationTimeLimit"}, \code{"MaxDepth"}, and \code{"Extension"}: a list of
exact algebraic numbers generating a subfield over which $p$ is factored first, the
factor containing the input being solved by the classical formulas after the generators
are themselves expressed in radicals (Proposition~\ref{prop:pairsum} with the generators
supplied instead of searched for; this is how the notebook's target is handled in
Section~\ref{sec:notebook}).  \code{RadicalExpressionQ} and \code{RadicalDepth}
implement the grammar of Section~\ref{sec:conventions}.
\end{sloppypar}
...
The regression suites were executed: \code{RunTests.wl} runs $67$ tests in $89$ s of
wall time (including the forced descents on both examples and all their conjugates);
\code{test\_roottoradicals.py} runs $48$ cases in $39$ s.  \code{verify\_wolfram.py}
re-verifies $13$ Python expressions exactly in a Wolfram kernel with \code{RootReduce},
identifying each conjugate by value; all pass.
...
project.
\item \code{ResourceLimit} asserts nothing about solvability.  Nonsolvable polynomials
whose Frobenius cycle types (within the first sixty good primes) show none of the
obstructions of Section~\ref{sec:negative} and whose group is larger than the limit end
here; so do solvable ones with $|H|\varphi(m)$ beyond the limit.
