...
arbitrary-length multiplicative problem is genuinely different: optimal factors may lie outside the
splitting field, two-factor optima are not many-factor optima, and no complete algorithm is known.  We
...
is such an example.  The accompanying Wolfram Language package and its Python counterpart implement all
algorithms with a new engine (numerical resolvents for the Galois group, a trace-form basis of the
splitting field) that avoids the impractically slow number-field conversions proposed in the reports;
every returned identity is verified exactly.
\end{abstract}
...
\item \emph{Many factors} (Section~\ref{sec:many}).  $\Dt$ can be smaller than the two-factor
  minimum: $(1+\sqrt2)(1+\sqrt3)(1+\sqrt5)$ has $\Dt=2$ but no two factors of degree below $8$.
  ...
  necessary norm condition, a sufficient radical condition, an exact rank-one tensor criterion for
  linearly disjoint families, and state the open problem.
...
\item \emph{Implementation} (Section~\ref{sec:impl}).  The Galois group is computed by numerical
  ...
  and all decision problems above become exact rational linear algebra.  A fast path works inside
  $\Q(a)$ without the Galois group.  The Wolfram package answers the two examples in about a second
  (fast path) and certifies global optimality; the Python port (python-flint, Arb ball arithmetic)
  computes the full splitting-field data in about three seconds, against about twenty seconds in the
  Wolfram kernel.
\end{enumerate}
...
Mathematica's \code{Root[f,k]} denotes the $k$th root of $f$ in the following order: real roots first,
increasing; then the non-real roots in conjugate pairs, ordered by increasing real part and then by
increasing $|\mathrm{Im}|$, the root of negative imaginary part first within a pair.  The Python code
uses the same order so that both implementations index roots identically.
...
$f$ and $g$ gives zero modulo $P_\times$, respectively $P_+$.  (The package runs these checks; all
identities in this article were verified in Wolfram 15.0.1.)  They show $u,v\in\Q(\alpha_\times)$ and
...
$d$, the exponent of $\prod S_{m_i}$ is $\lcm(1,\dots,d)$ (orders of permutations are lcm's of cycle
...
$2^{1/4}$ has $\Dp=4$ while its degree has prime bound $2$ and $D_4$ has exponent $4$; and they are not
sufficient for products even when the Galois group is elementary abelian (Theorem~\ref{thm:sign}).
...
\begin{align*}
 ...
 &=\Root[4-243\#+81\#^2-729\#^3+486\#^4+729\#^6\&,1]+\Root[4-243\#+\dots+729\#^6\&,2],
\end{align*}
...
gives $\Dp(s)=4$.  The package returns $\Root[-1+\#+\#^4\&,1]+\Root[-1+\#+\#^4\&,3]$ for
$\Root[-1+4\#^2+\#^6\&,3]$.
\end{example}
...
$e'\ne0$, the element $\eta/c=e\eta+e'i\eta$ has nonzero coordinates on $i\sqrt2,i\sqrt3,i\sqrt6$ and,
if $e\ne0$, on $\sqrt2,\sqrt3,\sqrt6$; in either case it generates $L'$ (from $i\sqrt2$ and $i\sqrt3$
one obtains $\sqrt6$, $\sqrt3$ and $i$), so it has degree $8$, and Theorem~\ref{thm:two} with $n=8$,
...
change: the prime $2$ becomes free, so only prime divisors $\ge3$ of $n$ bound $d$, and the exponent
condition becomes $\exp(G')\mid2\lcm(1,\dots,d)$.  The binary Gaussian problem $a=cX+c'Y$ is, by
...
computes the two-factor minimum inside $K$; if $K$ is Galois, it computes $\Dtt(a)$.
\end{corollary}
...
$L$ of $a$ of degree less than $8$ equals $a$; the optimal factors lie outside $L$, and the criterion
\eqref{eq:two} holds only with $t=2$.
\end{theorem}
...
$B_1\cap\R=B_0\not\ni a$, $[L:\Q]=16$.  The conjugates of $a$ are $\pm a,\pm iB/a,\pm irA/a,\pm r/a$
(each squares to a conjugate of $AB$: e.g.\ $(iB/a)^2=-B^2/(AB)=-B/A=B(1-r)/(-1)\cdot$, a conjugate of
$AB$), all in $L$; conversely the splitting field contains $a\cdot iB/a=iB$, hence $i$, and $B_0$, so
...
of degree at most $3$ cannot work: their compositum has degree at most $9$, is divisible by $8$ only if
it is $8$, which is impossible for a compositum of fields of degrees $\le3$ (degrees $\le 3\cdot3=9$
and $8$ requires a quadratic and a quartic, or two quartics, and quartics are excluded).  So
$\Dtt(\eta_3)=4$, attained e.g.\ by $(1+\sqrt2)(1+\sqrt3)\cdot(1+\sqrt5)$.
\end{example}

\begin{example}\label{ex:w}
$w=(1+\sqrt2)(1+\sqrt3)(1+\sqrt6)=7+4\sqrt2+3\sqrt3+2\sqrt6$ has degree $4$ and minimal polynomial
$x^4-28x^3+128x^2-200x+100$.  $\Dt(w)=2$, but $\Dtt(w)=4$: in $L=\Q(\sqrt2,\sqrt3)$ with $d=3$ the
exponent is $t=1$ (as in Theorem~\ref{thm:flat}), and for the three pairs of quadratic subfields the
...
\begin{problem}
Is $\Dt(a)$ computable?  Equivalently, given a Galois number field $L$, $a\in L^\times$ and $d$, decide
...
(units, class groups, $S$-units), not about vector spaces, and no finite criterion is known to us.  A
positive semi-decision procedure exists (Section~\ref{sec:resultants}); what is missing is a
terminating negative certificate.
\end{problem}
...
what makes the open problem hard.
...
Let $K$ be a number field and $E_1,\dots,E_r\subseteq K$ subfields with $\prod_i[E_i:\Q]=[K:\Q]$ such
...
rank one iff $c_{j_1\cdots j_r}=c_p\prod_i c_{p(j_i\to i)}/c_p$ for all indices, where $p(j\to i)$ is $p$
with its $i$th index replaced by $j$; the factors are then read off from the slices through $p$.
\end{proposition}
...
$A_\times,B_\times,A_+$; the sextic pair-product example $r_1r_2$ with $\Dt^{\Q(u)}=6>4=\Dt$; the
...
\mathrm{Val}^\top\mathrm{Val}$ (rounded rigorously), nondegenerate since $L/\Q$ is separable, and the
coordinates of any $y\in L$ given by its conjugate vector $(\sigma(y))_\sigma$ are
$\mathrm{Gram}^{-1}(\Tr(yb_l))_l$, again from rounded integer traces.  This yields, by exact rational
...
a \Root{} object by computing its distinct conjugates (the orbit of $v$ under the $A_\sigma$), the
integer polynomial with those roots, and the index of the root nearest to its value.  A consistency
check ($A_\sigma$ permutes the root coordinates as $\sigma$ does) guards the numerical steps, and every
decomposition returned is verified with \code{RootReduce} and \code{MinimalPolynomial}.

\subsection{The fast path inside $\Q(a)$}
...
\cite{SvH,r5,r9}.  In $K$ the additive criterion is complete for sums inside $K$, and the two-factor
criterion with $t=1$ is complete for two factors inside $K$; if $K$ is Galois (all $g_i$ linear) both
are globally complete.  The implementations try this path first and stop when it reaches a lower bound,
...
rational mean of the conjugates, $\Tr(y)/[L:\Q]$, is subtracted and collected into one rational term);
this recovers the trace-zero cubics of \eqref{eq:answers}.  For products the null vector is taken from
...
\end{lstlisting}
Results are associations with \code{"Terms"}, \code{"Degrees"}, \code{"MaximumDegree"},
\code{"LowerBound"}, \code{"Optimal"} (globally optimal, certified), \code{"ScopeOptimal"} (optimal
within the class searched), \code{"TwoFactorOptimal"}, \code{"NormExponent"}, \code{"Verified"},
\code{"Method"} and an inactive \code{"Expression"}.  Options: \code{"Scope" -> "InputField"} restricts
components to $\Q(a)$; \code{"Engine" -> "SplittingField"} forces the Galois route;
\code{"Coefficients" -> "GaussianRationals"} and \code{"MaxTerms"} implement Section~\ref{sec:binary};
\code{"MaxFactors" -> 2} restricts to the complete two-factor algorithm; \code{"MaxGroupOrder"} and
\code{"WorkingPrecision"} bound the Galois computation.  The Python module \code{rootdecomp.py} exposes
\code{sum\_decomposition}, \code{product\_decomposition}, \code{galois\_data}, \code{lower\_bound} and
\code{parse\_wolfram\_root}, with the same semantics and root numbering.

\subsection{Measurements}

Table~\ref{tab:timing} lists wall-clock times on the development machine (Wolfram 15.0.1, Python 3.14
with python-flint 0.8).  The Galois route of the Python port is faster than the Wolfram kernel because
...
Task & Wolfram & Python\\
\midrule
Splitting-field data of the degree-9 examples ($|G|=36$) & 18--21 s & 3 s\\
Product example, fast path in $\Q(a)$, certified optimal & $\approx1$ s & --\\
Sum example, fast path in $\Q(a)$, certified optimal & $\approx1$ s & --\\
$\Dp(\alpha_\times)=6$ with the splitting-field data cached & 0.2 s & 0.15 s\\
$\sqrt{(1+\sqrt2)(1+\sqrt3)}$, $|G|=16$, two-factor with $t=2$ & 4 s & 0.4 s\\
$x^4-x-1$ pair sum, $|G|=24$ & 3 s & 0.7 s\\
\code{ToNumberField[roots, All]} for the degree-9 examples & 14 min & --\\
\bottomrule
\end{tabular}
\caption{Timings; the fast-path figures are for the Wolfram package.}\label{tab:timing}
\end{table}

\subsection{Limits and scope of the certificates}

The complete algorithms need the Galois group; the resolvent method is practical for groups of order
...
factors, but global negative statements are unavailable unless a lower bound is attained.  The Python
Galois computation is rigorous (ball arithmetic and exact factorization); the Wolfram one relies on
exact minimal polynomials, numerical root matching with generous margins, and the group-closure and
automorphism-consistency checks, and every positive answer is verified exactly.  A returned
\code{"Optimal" -> False} means \emph{not certified}, not \emph{suboptimal}.
...
invisible to the Galois group, and its decidability is open.  For the two numbers of the question the
