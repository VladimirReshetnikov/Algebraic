"""
algebraic.radicals -- radical expressions for algebraic numbers with a solvable
Galois group.
...
integer polynomial with a root index, see root_decomposition.AlgebraicNumber), the
...
* the general Galois-Kummer descent (complete for solvable inputs): Galois
  group and splitting field from root_decomposition.galois_data (numerical resolvents
...
Requires python-flint >= 0.8, SymPy >= 1.14 and the root_decomposition module of the
...
from . import root_decomposition as rd
from .root_decomposition import AlgebraicNumber, PrecisionError
